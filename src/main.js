...
import {SEARCH_RADII_METERS,validPoint,panoramaSearchUrl,nearbyPanoramas,panoramaxUrl,panoramaxCoverageUrl,mapillaryUrl,osmSearchUrl,nominatimSearchUrl,locationCandidates,distanceMeters} from './field-view.js';
...
function renderPlaces() {
 ...
 <div class="place-actions"><button class="btn primary" data-action="streetview">${icon('compass')} 360° 프리뷰 열기</button>${external(osmSearchUrl(p.query),'OpenStreetMap에서 찾기')}</div>
 </article>
 <section class="media-section" aria-labelledby="media-title"><div class="section-head"><div><p class="eyebrow">EXPLORE THE PLACE</p><h3 id="media-title">현장 둘러보기</h3></div><span class="live-tag">360°</span></div><div id="media-stage" class="media-stage"></div><p class="media-help">Panoramax의 공개 360° 사진을 확인합니다. 사진이 있는 곳에서는 화면을 드래그해 회전하고 확대할 수 있습니다. 당시 건물을 복원한 3D 모델은 아닙니다.</p></section>
...
function freeViewActions(p){
 const otherLocations=state.viewCandidates.length>1?`<button class="btn" data-action="show-candidates">${icon('pin')} 다른 검색 위치</button>`:'';
 return `<div class="stage-actions"><button class="btn primary" data-action="streetview">${icon('compass')} 위치 다시 검색</button>${otherLocations}<button class="btn" data-action="choose-map-point">${icon('pin')} 지도에서 위치 지정</button>${external(osmSearchUrl(p.query),'OSM 주소 검색')}${external(googleMapsUrl(p),'Google Maps 웹에서 찾기')}${external(mapillaryUrl(state.viewPoint||city().center),'Mapillary 사진 지도')}</div>`;
}
function renderMedia() {
 const stage=document.getElementById('media-stage');if(!stage)return;
 if(state.mediaStatus==='loading'){
 stage.innerHTML=`<div class="stage-message"><span class="spinner" aria-hidden="true"></span><strong>360° 현장 자료를 확인하고 있습니다</strong><p>검색 위치 주변의 공개 파노라마 제공 여부를 확인합니다.</p></div>`;
 return;
 }
 if(state.mediaStatus==='ready'||state.mediaStatus==='coverage')return;
 const p=place();
 if(state.mediaStatus==='candidates'){
 stage.innerHTML=`<div class="stage-message"><strong>탐색할 위치를 선택하세요</strong><p>주소 검색 후보입니다. 역사적 건물의 정확한 위치로 검증된 것은 아닐 수 있습니다.</p><div class="location-candidates">${state.viewCandidates.map((item,i)=>`<button class="location-candidate" data-candidate="${i}"><strong>${esc(item.name)}</strong><small>이 위치의 360° 자료 확인 →</small></button>`).join('')}</div>${freeViewActions(p)}</div>`;
 return;
 }
 const message=state.mediaStatus==='unavailable'
 ?'이 위치 주변에서 자동으로 연결할 공개 360° 사진을 찾지 못했습니다. 지도에서 위치를 직접 지정하거나 외부 사진 지도를 확인할 수 있습니다.'
 :'공개 사진 서비스에 연결하지 못했습니다. 잠시 뒤 다시 시도하거나 외부 지도를 이용하세요.';
 const body=state.mediaStatus==='idle'
 ?'“360° 프리뷰 열기”를 누르면 장소를 찾은 뒤 주변의 공개 파노라마를 확인합니다.'
 :message;
 stage.innerHTML=`<div class="stage-message"><div class="stage-emblem">${icon('compass')}</div><strong>${state.mediaStatus==='idle'?'360° 현장 프리뷰':'현장 보기 안내'}</strong><p>${esc(body)}</p>${freeViewActions(p)}</div>`;
}
...
function renderPanoramaxFrame(url,title,note,status='ready'){
 const stage=document.getElementById('media-stage');if(!stage)return;
 state.mediaStatus=status;
 stage.innerHTML=`<div class="pano-header"><span>${esc(title)}</span>${external(url,'새 창에서 크게 보기','secondary')}</div>
 <div class="pano-container pano-iframe-wrap">
 <iframe class="pano-iframe" src="${esc(url)}" title="${esc(title)}" loading="eager" allow="fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>
 </div>
 <p class="pano-note">${esc(note)}</p>${freeViewActions(place())}`;
}
async function openStreetView(){
 ...
 if(!candidates.length){state.mediaStatus='unavailable';renderMedia();return;}
 const candidate=candidates[0];
 state.viewPoint=candidate.point;state.viewName=candidate.name;
 showSitePosition(candidate.point,'주소 검색 위치');
 await searchPanoramas(candidate.point,candidate.name);
...
async function searchPanoramas(point,name){
 ...
 try{
 let photos=[],usedRadius=SEARCH_RADII_METERS[0];
 for(const radius of SEARCH_RADII_METERS){
 usedRadius=radius;
 const response=await fetch(panoramaSearchUrl(point,radius),{headers:{Accept:'application/geo+json, application/json'}});
 if(!response.ok)continue;
 const result=await response.json();
 photos=nearbyPanoramas(result.features,point,radius);
 if(photos.length)break;
 }
 if(token!==state.stageToken||state.screen!=='places')return;
 if(!photos.length){
 const coverage=panoramaxCoverageUrl(point,17);
 renderPanoramaxFrame(
 coverage,
 'Panoramax · 360° 탐색 지도',
 `${name} 주변에서 바로 연결할 360° 사진을 찾지 못했습니다. 아래 지도는 해당 위치 주변의 파노라마 제공 범위를 확인하기 위한 화면입니다.`,
 'coverage'
 );
 showSitePosition(point,'360° 탐색 기준 위치');
 return;
 }
 const photo=photos[0];
 state.viewPhoto=photo;
 const photoPoint=[photo.geometry.coordinates[1],photo.geometry.coordinates[0]];
 const distance=Math.round(distanceMeters(point,photoPoint));
 const url=panoramaxUrl(photo);
 renderPanoramaxFrame(
 url,
 'Panoramax · 공개 360° 사진',
 `${name} 기준 약 ${distance}m 거리에서 검색된 현재 사진입니다. 화면을 드래그해 회전하고 확대할 수 있으며, 역사적 건물의 원형이나 내부를 복원한 영상은 아닙니다.`,
 'ready'
 );
 showSitePosition(photoPoint,'360° 사진 촬영 위치');
 }catch(error){
 if(token!==state.stageToken)return;
 try{
 const coverage=panoramaxCoverageUrl(point,17);
 renderPanoramaxFrame(
 coverage,
 'Panoramax · 360° 탐색 지도',
 '자동 검색 연결이 원활하지 않아 탐색 지도를 표시했습니다. 지도에서 주변의 파노라마 자료를 직접 확인할 수 있습니다.',
 'coverage'
 );
 }catch(_){state.mediaStatus='error';renderMedia();}
 }
}
function showCandidateChoices(){
 if(!state.viewCandidates.length){openStreetView();return;}
 state.mediaStatus='candidates';
 renderMedia();
}
...
function registerEvents() {
 ...
 else if(action==='show-candidates')showCandidateChoices();
