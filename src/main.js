...
function renderShell(){
  ...
  app.innerHTML=`
    <div class="app-shell">
      <header class="topbar">
        <a class="brand" href="#" data-action="home" aria-label="처음 화면으로 이동" title="처음 화면으로 이동">
...
function returnToIntro(){
  // 진행 중인 위치/영상 요청의 결과가 뒤늦게 화면을 바꾸지 못하게 무효화합니다.
  state.requestToken++;
  state.youtubeRequestToken++;
  state.transitioning=false;
  hideMoveTransition();

  // 현재 지도를 정리한 뒤 첫 화면 상태로 초기화합니다.
  if(map){
    map.remove();
    map=null;
  }
  ...
  state.cityId='shanghai';
  state.placeId='shanghai-office';
  state.point=null;
  state.pointLabel='';
  state.pointAccuracy='';
  state.videoId=null;
...
function focusPoint(animated=false){
  ...
  const duration=animated?.75:.3;
  map.flyTo(point,validPoint(state.point)?16:city().zoom,{duration});
  if(placeMarker){
    if(animated){
      map.once('moveend',()=>placeMarker?.openPopup());
    }else{
...
function setPlaceMarker(point,label){
  ...
  placeMarker.bindPopup(
    `<div class="map-popup"><small>${esc(city().name)} · ${esc(place().period)}</small><strong>${esc(place().name)}</strong><span>${esc(label)}</span></div>`,
    {autoPan:true,keepInView:true,maxWidth:280,minWidth:180,autoPanPadding:[28,28]}
  );
