// Public, read-only street-level imagery helpers. No Google Maps SDK or API key.
export const PANORAMAX_API = 'https://api.panoramax.xyz/api';
export const PANORAMAX_WEB = 'https://api.panoramax.xyz/';
export const SEARCH_RADIUS_METERS = 250;
export const SEARCH_RADII_METERS = [160, 450, 900];
...
export function isPanorama(feature) {
  ...
  const type=String(p['geovisio:type']||p['panoramax:type']||'').toLowerCase();
  const projection=String(p['exif:projection_type']||p['projection_type']||'').toLowerCase();
  return Number(orientation.field_of_view)>=350
    || type==='equirectangular' || type==='spherical'
    || projection==='equirectangular' || projection==='spherical';
}
export function nearbyPanoramas(features,point,radius=SEARCH_RADIUS_METERS) {
  return (Array.isArray(features)?features:[]).filter(f=>{
    ...
    return isPanorama(f)&&typeof f.id==='string'&&typeof f.collection==='string'
      &&distanceMeters(point,pos)<=radius;
  }).sort((a,b)=>distanceMeters(point,[a.geometry.coordinates[1],a.geometry.coordinates[0]])
    -distanceMeters(point,[b.geometry.coordinates[1],b.geometry.coordinates[0]]));
}
export function panoramaxUrl(photo){
  const u=new URL(PANORAMAX_WEB);
  if(photo?.id)u.searchParams.set('pic',photo.id);
  u.searchParams.set('focus','pic');
  u.searchParams.set('nav','seq');
  return u.href;
}
export function panoramaxCoverageUrl(point,zoom=17){
  if(!validPoint(point))throw new Error('INVALID_POINT');
  const u=new URL(PANORAMAX_WEB);
  u.searchParams.set('map',`${zoom}/${point[0]}/${point[1]}`);
  u.searchParams.set('focus','map');
  u.searchParams.set('pic_type','equirectangular');
  return u.href;
}
...
export function locationCandidates(rows,center,maxDistance=30000){
  ...
  })).filter(row=>validPoint(row.point)&&distanceMeters(center,row.point)<=maxDistance);
}
